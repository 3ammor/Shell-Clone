...
const int csize = 40; list<string> commands; list<string>::iterator it;             //<Linked List> Datastructure is used to store the commands.
...
void change_signal_handlers();                                //Function that changes the SIGINT,SIGCHLD handlers.
void int_signal_handler(int signum);                         //Handler for SIGINT signal.
void child_signal_handler(int signum);                          //This handler reaps the info. generated by stopped or killed children to avoid being transformed into zombies.
...
void change_signal_handlers()                                //Change the SIGINT,SIGCHLD handlers in the shell (parent) process.
{
    //SIGINT
    struct sigaction sa;
...
void child_signal_handler(int signum) {
     ...
     while (waitpid((pid_t)(-1), 0, WNOHANG) > 0);      //Reap all zombies until no zombies exist.
     errno = saved_errno;
}
